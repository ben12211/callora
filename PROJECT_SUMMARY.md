...
עדכון אחרון: 2026-08-20 · ענף: `claude/project-summary-md-2a5dae` · גרסה: `0.1.0` (חבילה: `callora-backend`)
...
**עודכן:** מודל ההרשאות כבר אינו שטוח. לכל מנהל יש `role` — `platform` או `business` — ומנהל עסק מוגבל לעסק אחד בלבד: הוא רואה רק אותו, את השיחות והתמלולים שלו, ואת רשומות הביקורת עליו. יצירת עסק ומחיקתו, דף הספקים ו-`/metrics` נשארים ברמת הפלטפורמה. ברירת המחדל היא `platform`, ולכן כל חשבון קיים שומר בדיוק על ההרשאות שהיו לו.
