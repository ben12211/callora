...
const TEENS_M: [&str; 10] = [
    "עשרה", "אחד עשר", "שנים עשר", "שלושה עשר", "ארבעה עשר", "חמישה עשר", "שישה עשר", "שבעה עשר", "שמונה עשר",
    "תשעה עשר",
];
const TEENS_F: [&str; 10] = [
    "עשר", "אחת עשרה", "שתים עשרה", "שלוש עשרה", "ארבע עשרה", "חמש עשרה", "שש עשרה", "שבע עשרה", "שמונה עשרה",
    "תשע עשרה",
];
...
const THOUSANDS: [&str; 10] = [
    "", "אלף", "אלפיים", "שלושת אלפים", "ארבעת אלפים", "חמשת אלפים", "ששת אלפים", "שבעת אלפים", "שמונת אלפים",
    "תשעת אלפים",
];
...
pub fn find_numbers(tokens: &[&str]) -> Vec<FoundNumber> {
    ...
    while i < tokens.len() {
        ...
        while j < tokens.len() {
            ...
            let p = if j > start {
                piece(tok).or_else(|| tok.strip_prefix('ו').and_then(piece))
            } else {
                piece(tok)
            };
            match p {
